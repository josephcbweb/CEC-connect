...
function main() {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        let cse = yield prisma_1.prisma.department.findFirst({
            where: {
                OR: [
        ...
        if (!cse) {
            cse = yield prisma_1.prisma.department.create({
                data: { name: "computer science and engineering", department_code: "CSE" }
            });
        }
        let ece = yield prisma_1.prisma.department.findFirst({
            where: {
                OR: [
        ...
        if (!ece) {
            ece = yield prisma_1.prisma.department.create({
        ...
        for (const s of students) {
            const exists = yield prisma_1.prisma.student.findFirst({ where: { email: s.email } });
            if (!exists) {
                yield prisma_1.prisma.student.create({ data: s });
