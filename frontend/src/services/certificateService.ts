...
export const certificateService = {
  ...
  ): Promise<Certificate & { warning?: string }> => {
    try {
      console.log("Submitting certificate request:", data);

      const response = await axios.post(`${API_BASE}/api/certificates`, data, {
      ...
      if (axios.isAxiosError(error)) {
        ...
        if (serverError) {
          ...
          const errorMessage =
            errorCode === "NEXT_APPROVER_MISSING" ? serverError : serverError;
  ...
  processCertificate: async (
    ...
    try {
      ...
      if (axios.isAxiosError(error)) {
        ...
        throw new Error(
          axiosError.response?.data?.error || "Failed to process certificate",
        );
      }
  ...
  getWorkflowStatus: async (id: number): Promise<Certificate> => {
    try {
      ...
      if (axios.isAxiosError(error)) {
        throw new Error(
          error.response?.data?.error || "Failed to fetch workflow status",
        );
      }
