...
const BusServiceManager = () => {
  ...
  const fetchStudentData = async () => {
    try {
      const studentId =
        studentData?.id ||
        JSON.parse(localStorage.getItem("studentAuthToken") || "{}").userId;

      if (!studentId) return;

      const profileRes = await axios.get(
        `http://localhost:3000/students/profile/${studentId}`,
      );
      setStudent(profileRes.data);

      // Also fetch buses
      const busesRes = await axios.get(
        "http://localhost:3000/students/bus/routes",
      );
      ...
      <div className="max-w-4xl mx-auto p-6">
        <div className="bg-red-50 border-2 border-red-200 rounded-3xl overflow-hidden">
          ...
          <div className="bg-red-600 p-6 text-white flex items-center gap-4">
            ...
            <div>
              ...
              <p className="opacity-90 text-sm">
                Bus facility temporarily disabled
              </p>
            </div>
          </div>
          ...
          <div className="p-8 space-y-6">
            <div className="bg-white rounded-2xl p-6 border border-red-100 flex items-start gap-4">
              ...
              <div>
                <h3 className="font-semibold text-gray-800 text-lg">
                  Overdue Dues Detected
                </h3>
                <p className="text-gray-600 mt-1">
                  Your bus pass for <strong>{busName}</strong> ({busNumber}) at{" "}
                  <strong>{stopName}</strong> has been suspended due to overdue
                  bus fees. Please clear your outstanding dues to reactivate
                  your pass.
                </p>
              </div>
            </div>
            ...
            <div className="bg-red-100/50 rounded-xl p-4 flex items-center gap-3 text-red-700 text-sm font-medium">
              <span className="w-2.5 h-2.5 rounded-full bg-red-500 animate-pulse"></span>
              Status: SUSPENDED — Contact the accounts office to resolve pending
              fees
            </div>
          </div>
        </div>
        ...
        <div className="bg-emerald-50 border border-emerald-200 rounded-3xl overflow-hidden">
          ...
          <div className="bg-emerald-600 p-6 text-white flex items-center gap-4">
            ...
            <div>
              ...
              <p className="opacity-90 text-sm">
                Complete the payment to activate your bus pass
              </p>
            </div>
          </div>
              ...
              <div className="space-y-3">
                <div className="flex justify-between items-center text-gray-600">
                  <span className="flex items-center gap-2">
                    <Bus className="w-4 h-4" /> Bus Route
                  </span>
            ...
            <div className="bg-amber-50 rounded-xl p-4 flex items-start gap-3 border border-amber-200">
              ...
              <div>
                ...
                <p className="text-amber-700 text-sm mt-1">
                  Please pay the bus fee at the college accounts office. Your
                  bus pass will be activated once the payment is verified by the
                  administration.
                </p>
              </div>
            </div>
      ...
      <div className="max-w-4xl mx-auto p-6">
        <div className="bg-yellow-50 border border-yellow-200 rounded-2xl p-8 text-center space-y-4">
          ...
          <p className="text-gray-600 max-w-lg mx-auto">
            You have requested for <strong>{busName}</strong> at{" "}
            <strong>{stopName}</strong>. Your application is currently under
        ...
        <div className="bg-white p-8 rounded-3xl shadow-sm border border-gray-100">
          <div className="mb-8">
            <h2 className="text-2xl font-bold text-[#009689]">
              Avail College Bus Facility
            </h2>
            <p className="text-gray-500 mt-1">
              Select your preferred route and stop to request bus service.
            </p>
          </div>
              ...
              <div>
                ...
                >
                  <Select
                    className="w-full h-12"
                    placeholder={
                      selectedBus ? "Choose your stop" : "Select a bus first"
                    }
              ...
              {activeBus && selectedStop ? (
                <div className="space-y-4 animate-in fade-in slide-in-from-bottom-4 duration-500">
                  ...
                  <div className="flex justify-between items-center text-gray-600">
                    ...
                    <span className="font-medium text-gray-900">
                      {
                        availableStops.find((s) => s.id === selectedStop)
                          ?.stopName
                      }
                    </span>
                  </div>
