  ...
  const handleReject = async (id: number) => {
    ...
    if (reason) {
      try {
        ...
        await certificateService.updateCertificateStatus(id, 'REJECTED', reason);
        loadCertificates();
      } catch (error) {
              ...
              <tr key={certificate.id}>
                <td className="px-6 py-4">
                  <div>
                    ...
                    <div className="text-sm text-gray-500">{certificate.student?.admission_number}</div>
                  </div>
                </td>
                <td className="px-6 py-4">{certificate.type.replace('_', ' ')}</td>
